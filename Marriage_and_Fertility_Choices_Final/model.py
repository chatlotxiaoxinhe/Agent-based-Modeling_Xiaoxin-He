...
def compute_within_type_moran(
    model: "MarriageFertilityModel",
    target_type: str,
    target_state: str,
) -> float:
    """
    Moran's I restricted to agents of a single ability type.

    Same formula as compute_moran_i, but the neighbor sum W and the
    deviation product are restricted to pairs of same-type agents
    (within {high, median, low}). Agents of other types are ignored
    as if they were empty cells.

    This strips out the type-region background effect that contaminates
    the basic Moran's I: at γ=0, agents of different types end up in
    red at very different rates by region, so basic Moran's I shows
    positive autocorrelation purely from type-region placement.
    The within-type variant asks the residual question:
    *within the same type's region, are reds clustered more than random?*

    A non-zero within-type Moran's I would be a direct signature of γ's
    local-comparison effect, since the type-region background is held fixed.
    """
    # Filter to target-type agents.
    agents = [a for a in model.agents if a.type == target_type]
    N = len(agents)
    if N < 2:
        return 0.0

    # Indicator vector and its mean (within type only).
    x = {a: (1.0 if a.state == target_state else 0.0) for a in agents}
    mean = sum(x.values()) / N
    dev = {a: x[a] - mean for a in agents}
    ...
    # Set membership for fast filtering of cross-type neighbors.
    target_set = set(agents)

    # Numerator: sum over ordered Moore-neighbor pairs where BOTH agents
    # are of target_type. Cross-type neighbors are skipped.
    numerator = 0.0
    W = 0
    for a in agents:
        for nbr_cell in a.cell.neighborhood:
            for b in nbr_cell.agents:
                if b is a or b not in target_set:
                    continue
                numerator += dev[a] * dev[b]
                W += 1

    if W == 0:
        # An agent in this type may have no same-type neighbors (edge of
        # the region). Return 0 as the neutral default.
        return 0.0

    return (N / W) * (numerator / denom)


def moran_i_red_within_high(model):
    return compute_within_type_moran(model, "high", RED)


def moran_i_red_within_median(model):
    return compute_within_type_moran(model, "median", RED)


def moran_i_red_within_low(model):
    return compute_within_type_moran(model, "low", RED)
    ...
    def __init__(
        ...
        self.datacollector = DataCollector(
            model_reporters={
                ...
                # Within-type Moran's I for RED: residual spatial autocorrelation
                # after stripping out the type-region background. A non-zero
                # value at γ>0 would be a direct signature of social-comparison
                # -driven local clustering.
                "MoransI_Red_within_high":   moran_i_red_within_high,
                "MoransI_Red_within_median": moran_i_red_within_median,
                "MoransI_Red_within_low":    moran_i_red_within_low,
            },
            agent_reporters={
